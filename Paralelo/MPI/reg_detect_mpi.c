                       ...
                       int start_iter, int end_iter)
{
    ...
    for (t = start_iter; t < end_iter; t++) {
        ...
        // Adicione uma barreira antes de continuar
        ...
        // Adicione outra barreira antes de continuar
        MPI_Barrier(MPI_COMM_WORLD);
...
int main(int argc, char **argv)
{
    ...
    int start_iter = rank * work_per_process;
    int end_iter = (rank == size - 1) ? niter : start_iter + work_per_process;
    ...
    /* Adicione uma barreira antes de imprimir os resultados */
    MPI_Barrier(MPI_COMM_WORLD);
